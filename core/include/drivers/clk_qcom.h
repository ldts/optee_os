...
struct qcom_lucidevo_pll_config {
	...
	uint32_t cal_l_val;
	uint32_t alpha_val;
	uint32_t pre_div;		/* div-1 to div-8; 0 means leave at div-1 */
	uint32_t config_ctl;
 ...
 * block starts at @pll_base. Returns TEE_ERROR_TIMEOUT if the PLL fails to lock.
 */
TEE_Result qcom_clock_lucidevo_pll_enable(vaddr_t pll_base,
