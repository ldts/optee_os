...
#define XRDC_SZ_SIZE(mod)		(uint32_t) ((1 << (mod + 1)) - 1)
...
static const struct mrc_setting mrc_setting_7ulp[] = {
	 ...
	 *
	 * Region 0: 0x1FFD.0000 - 0x1FFD.FFFF NS
	 * Region 1: 0x1FFE.0000 - 0x1FFF.FFFF NS (0x1FFFC000 - 0x1FFFFFFF disabled)
	 * Region 2: 0x1FFF.C000 - 0x1FFF.FFFF NS - (M4)
	 * Region 3: 0x2000.0000 - 0x2000.FFFF NS
	 */
	{MRC_M4_TCM, MRC_REGION_0,
		M4_SRAM_L_BASE,
		XRDC_REGION_SZ_64K,
		0,
		{
	 ...
	 * Quad SPI Interface
	 *
	 * Region 0: 0xC0000000 - 0xCFFFFFFF NS 256M
	 * Region 1: 0x24000000 - 0x00000080 NS 128B
	 * Region 2: 0x04000000 - 0x0BFFFFFF NS 128B
	 */
	{MRC_QSPI, MRC_REGION_0, QSPI_BASE, XRDC_REGION_SZ_256M, 0,
		{
			{MDA_DOMAIN_0, XRDC_POLICY_LVL7},
			{MDA_DOMAIN_1, XRDC_POLICY_LVL7},
			{(-1), 0}, /* End */
	...
	{MRC_QSPI, MRC_REGION_1, QSPI_BUFFERS, XRDC_REGION_SZ_128B, 0,
		{
			{MDA_DOMAIN_0, XRDC_POLICY_LVL7},
			{MDA_DOMAIN_1, XRDC_POLICY_LVL7},
			{(-1), 0}, /* End */
	...
	{MRC_QSPI, MRC_REGION_2, QSPI_ALIASING, XRDC_REGION_SZ_256M,
		XRDC_SUBREGION_DIS_0 | XRDC_SUBREGION_DIS_1 |
		XRDC_SUBREGION_DIS_6 | XRDC_SUBREGION_DIS_7,
		{
			{MDA_DOMAIN_0, XRDC_POLICY_LVL7},
			{MDA_DOMAIN_1, XRDC_POLICY_LVL0},
			{(-1), 0}, /* End */
	...
	/*
	 * Multi Mode DDR Controller: this configuration is tailored to our board
	 *
	 * Region 0: 0x60000000 - 0x7FFFFFFF NS
	 * Region 1: 0x80000000 - 0x8FFFFFFF NS
	 * Region 2: 0x90000000 - 0x97FFFFFF NS
	 * Region 3: 0x98000000 - 0x9FFFFFFF NS  (0x9E000000 - 0x9FFFFFFF disabled)
	 * Region 4: 0x9E000000 - 0x9FDFFFFF SEC (CFG_TZDRAM)
	 * Region 5: 0x9FE00000 - 0x9FFFFFFF NS  (CFG_SHMEM_START)
	 */
	{MRC_MMDC, MRC_REGION_0,
		DRAM0_BASE,
		XRDC_REGION_SZ_512M,
		0,
		{
	...
	{MRC_MMDC, MRC_REGION_1,
		DRAM0_BASE +
			XRDC_SZ_SIZE(XRDC_REGION_SZ_512M) + 1,
		XRDC_REGION_SZ_256M,
		0,
		{
			{MDA_DOMAIN_0, XRDC_POLICY_LVL7},
	...
	{MRC_MMDC, MRC_REGION_2,
		DRAM0_BASE +
			XRDC_SZ_SIZE(XRDC_REGION_SZ_512M) + 1 +
			XRDC_SZ_SIZE(XRDC_REGION_SZ_256M) + 1,
		XRDC_REGION_SZ_128M,
		0,
		{
			{MDA_DOMAIN_0, XRDC_POLICY_LVL7},
	...
	/* if CFG_SHMEM_START or CFG_TZDRAM_START change in location or
	 * size, these values will need to be adjusted
	 */
	{MRC_MMDC, MRC_REGION_3,
		DRAM0_BASE +
			XRDC_SZ_SIZE(XRDC_REGION_SZ_512M) + 1 +
			XRDC_SZ_SIZE(XRDC_REGION_SZ_256M) + 1 +
			XRDC_SZ_SIZE(XRDC_REGION_SZ_128M) + 1,
		XRDC_REGION_SZ_128M,
		/* exclude 0x9E000000 and 0x9F000000 */
		XRDC_SUBREGION_DIS_6 | XRDC_SUBREGION_DIS_7,
		{
			{MDA_DOMAIN_0, XRDC_POLICY_LVL7},
			{MDA_DOMAIN_1, XRDC_POLICY_LVL7},
			       ...
			       const struct mrc_setting *mrc)
{
	...
	if (mrc->base_addr & XRDC_SZ_SIZE(mrc->size)) {
		EMSG("Base address 0x%lx must be aligned with the size 0x%x",
			mrc->base_addr, XRDC_SZ_SIZE(mrc->size));
		return -1;
	}
...
static int xrdc_mrc_init(struct xrdc_reg_desc *xrdc_reg)
{
	...
	while (mrc->memory_slave >= 0) {
		EMSG("setting xrdc mrc slave: %d", mrc->memory_slave);
		ret |= xrdc_mrc_set_region(xrdc_reg, mrc);
		if (ret)
			EMSG(" xrdc mrc slave: %d failed", mrc->memory_slave);
		mrc++;
	}
