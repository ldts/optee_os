...
static TEE_Result do_encrypt(struct drvcrypt_rsa_ed *rsa_data)
{
	...
	struct versal_crypto_mem cipher = { };
	struct versal_crypto_mem key = { };
	struct versal_crypto_mem msg = { };
	TEE_Result ret = TEE_SUCCESS;
	struct cmd_args arg = { };

	versal_crypto_alloc(512 + crypto_bignum_num_bytes(p->e), NULL, &key);
	crypto_bignum_bn2bin_eswap(p->n, key.buf);
	crypto_bignum_bn2bin_eswap(p->e, (uint8_t *)key.buf + 512);

	versal_crypto_alloc(rsa_data->message.length, rsa_data->message.data,
			    &msg);
	versal_crypto_alloc(rsa_data->cipher.length, NULL, &cipher);
...
static TEE_Result do_decrypt(struct drvcrypt_rsa_ed *rsa_data)
{
	...
	versal_crypto_alloc(512 + crypto_bignum_num_bytes(p->d), NULL, &key);
	crypto_bignum_bn2bin_eswap(p->n, key.buf);
	crypto_bignum_bn2bin_eswap(p->d, (uint8_t *)key.buf + 512);

	versal_crypto_alloc(rsa_data->cipher.length, rsa_data->cipher.data,
			    &cipher);
	versal_crypto_alloc(rsa_data->message.length, NULL, &msg);
	versal_crypto_alloc(sizeof(*cmd), NULL, &cmd_buf);
