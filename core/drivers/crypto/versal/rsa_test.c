...
static void crypto_bignum_bin2bn_eswap(uint8_t *from, size_t len,
				       struct bignum *to)
{
	uint8_t *p = 0;
	uint8_t tmp = 0;
	...
	p = mempool_calloc(mempool_default, 1, len);
	if (!p)
		panic();
...
static TEE_Result test_sign(void)
{
	 ...
	 * by OP-TEE
	 */
	crypto_bignum_bin2bn_eswap(private_exp,
				   sizeof(private_exp), keypair.d);
	crypto_bignum_bin2bn_eswap(mod, sizeof(mod), keypair.n);
...
static TEE_Result test_verify(void)
{
	TEE_Result ret = TEE_SUCCESS;
	size_t signature_len = sizeof(signature);
	 ...
	 * by OP-TEE
	 */
	crypto_bignum_bin2bn_eswap((uint8_t *)&public_exp,
				   sizeof(public_exp), pubkey.e);
	crypto_bignum_bin2bn_eswap(mod, sizeof(mod), pubkey.n);
...
static struct {
	...
	{ .f = test_sign,    .name = STR(rsa sign), },
	{ .f = test_verify,  .name = STR(rsa verify), },
