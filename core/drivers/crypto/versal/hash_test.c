...
static TEE_Result test_sha3(void)
{
	...
	res = crypto_hash_alloc_ctx(&ctx, TEE_ALG_SHA3_384);
	if (res != TEE_SUCCESS) {
...
static TEE_Result test_sha3_0(void)
{
	...
	crypto_hash_alloc_ctx(&ctx1, TEE_ALG_SHA3_384);
	crypto_hash_alloc_ctx(&ctx2, TEE_ALG_SHA3_384);
	crypto_hash_alloc_ctx(&ctx3, TEE_ALG_SHA3_384);
