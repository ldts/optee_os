...
static TEE_Result do_init(struct drvcrypt_authenc_init *dinit)
{
	...
	struct versal_crypto_mem init_buf = { };
	...
	/* write the key */
	versal_crypto_alloc(dinit->key.length, dinit->key.data, &p);
...
init_op:
	...
	versal_crypto_alloc(sizeof(*init), NULL, &init_buf);
	versal_crypto_alloc(dinit->nonce.length, dinit->nonce.data, &p);
...
static TEE_Result do_update_aad(struct drvcrypt_authenc_update_aad *dupdate)
{
	...
	versal_crypto_alloc(dupdate->aad.length, dupdate->aad.data, &p);
				 ...
				 *dupdate, bool is_last)
{
	...
	struct versal_crypto_mem input_cmd = { };
	struct versal_crypto_mem p = { };
	struct versal_crypto_mem q = { };
	TEE_Result ret = TEE_SUCCESS;
	struct cmd_args arg = { };

	versal_crypto_alloc(dupdate->src.length, dupdate->src.data, &p);
	versal_crypto_alloc(dupdate->dst.length, NULL, &q);
	versal_crypto_alloc(sizeof(*input), NULL, &input_cmd);
...
static TEE_Result do_enc_final(struct drvcrypt_authenc_final *dfinal)
{
	...
	versal_crypto_alloc(GCM_TAG_LEN, NULL, &p);
...
static TEE_Result do_dec_final(struct drvcrypt_authenc_final *dfinal)
{
	...
	versal_crypto_alloc(dfinal->tag.length, dfinal->tag.data, &p);
	arg.ibuf[0].buf = p.buf;
	arg.ibuf[0].len = p.alloc_len;
