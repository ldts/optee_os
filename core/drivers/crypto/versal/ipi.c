...
TEE_Result versal_crypto_alloc(size_t len, const void *init,
			       struct versal_crypto_mem *mem)
{
				 ...
				 struct cmd_args *arg)
{
	...
	cmd.data[i++] = virt_to_phys(arg->ibuf[0].buf);
	cmd.data[i++] = virt_to_phys(arg->ibuf[0].buf) >> 32;
	...
	cmd.data[i++] = virt_to_phys(arg->ibuf[1].buf);
	cmd.data[i++] = virt_to_phys(arg->ibuf[1].buf) >> 32;
cache:
	for (i = 0; i < MAX_IPI_BUF; i++) {
		cmd.ibuf[i].len = arg->ibuf[i].len;
