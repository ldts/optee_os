...
static TEE_Result stm32mp15_read_uid(uint32_t *uid)
{
	...
	if (stm32_bsec_shadow_read_otp(q++, UID0_OTP))
		return TEE_ERROR_GENERIC;

	if (stm32_bsec_shadow_read_otp(q++, UID1_OTP))
		return TEE_ERROR_GENERIC;

	if (stm32_bsec_shadow_read_otp(q++, UID2_OTP))
		return TEE_ERROR_GENERIC;
...
TEE_Result tee_otp_get_hw_unique_key(struct tee_hw_unique_key *hwkey)
{
	...
	key = mempool_calloc(mempool_default, 1, len);
	if (!key)
	...
	ret = aes_gcm_encrypt_uid(p, len, hwkey->data, &len);
