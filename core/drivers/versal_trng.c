...
#define TRNG_SWAP_ENDIAN	TEE_U32_BSWAP
...
static void setup_key(const unsigned char *k, int klen)
{
	unsigned char rcon = 1;
	int i, sch_size = 240;

	rounds = 14;
	memcpy(schedule, k, (unsigned int)klen);
...
static void trng_df_init(bool *init)
{
	uint8_t sb[] = {
...
{
	static bool df_init;
	const uint8_t DFKey[DF_KEY_LEN] = {
	...
	if (!df_init)
		trng_df_init(&df_init);

	trng->dfin.val2 = flag == DF_SEED ?
			  TRNG_SWAP_ENDIAN(TRNG_PERS_STR_LEN) :
			  TRNG_SWAP_ENDIAN(TRNG_GEN_LEN);
	...
	if (!pstr) {
		...
		trng->dfin.val1 = TRNG_SWAP_ENDIAN(trng->len);
	...
	for (index = 0; index < TRNG_SEED_LEN; index += BLK_SIZE) {
		inp_blk = index == 0 ? &dfout[TRNG_SEC_STRENGTH_LEN] :
				       &dfout[index - BLK_SIZE];
		out_blk = &dfout[index];
		encrypt(inp_blk, out_blk);
	}
}

static uint32_t trng_read32(vaddr_t addr, uint32_t off)
{
	return io_read32(addr + off);
}

static void trng_write32(vaddr_t addr, uint32_t off, uint32_t val)
{
	io_write32(addr + off, val);
}

static void trng_clrset32(vaddr_t addr, uint32_t off, uint32_t mask,
			  uint32_t val)
{
			       ...
			       uint32_t num_regs, const uint8_t *buf)
{
	...
	for (i = 0; i < num_regs; ++i) {
		...
		for (val = 0, cnt = 0; cnt < TRNG_BYTES_PER_REG; ++cnt)
...
static TEE_Result trng_wait_for_event(vaddr_t addr, uint32_t off,
				      uint32_t mask, uint32_t event,
				      uint32_t time_out)
{
				   ...
				   const struct trng_usr_cfg *usr_cfg)
{
	...
	seed = trng->usr_cfg.iseed_en ?
	       (void *)trng->usr_cfg.init_seed : NULL;
	pers = trng->usr_cfg.pstr_en ?
	       (void *)trng->usr_cfg.pstr : NULL;
