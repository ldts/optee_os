 ...
 *
 * Per-target QUP clock config for Lemans (Hoya family): the per-SE serial-
 ...
 * the walker resolves each against the domain's regmap.
 */
...
/* Lemans routes every QUP register through the single central GCC block. */
static struct qcom_clk_regmap gcc_regmap = {
	.io.pa = GCC_BASE,
	.size = GCC_SIZE,
};
...
static const struct qcom_clk_src_vote qup_se_src_votes[] = {
	...
	{ &gcc_regmap, MUX_GPLL4, GCC_PLL_BRANCH_ENA_VOTE,
...
static const struct qcom_clk_cfg lemans_clk_cfg = {
