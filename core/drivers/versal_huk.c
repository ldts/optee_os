...
#if TEST_KEY_UINT8_T
#define TEST_KEY { \
0xf8, 0x78, 0xb8, 0x38, 0xd8, 0x58, 0x98, 0x18, 0xe8, 0x68, 0xa8, \
0x28, 0xc8, 0x48, 0x88, 0x08, 0xf0, 0x70, 0xb0, 0x30, 0xd0, 0x50, \
0x90, 0x10, 0xe0, 0x60, 0xa0, 0x20, 0xc0, 0x40, 0x80, 0x00, }
#else
/* notice: the PLM is little endian so if programming the keys in uint32_t
 * they have to be BE swapped
 */
#define TEST_KEY { \
...
static int versal_secure_key(enum aes_key_src src)
{
	...
	case XSECURE_AES_EFUSE_USER_KEY_0:
		if (0) {
			/* TODO for AMD
			 * need to guarantee that the eFuse has been written
			 */
	...
	case XSECURE_AES_EFUSE_USER_KEY_1:
		if (0) {
			 ...
			 * need to guarantee that the eFuse has been written
			 */
			key = "XSECURE_AES_EFUSE_USER_KEY_1";
			goto error;
		}
		return true;
	...
	case XSECURE_AES_PUF_KEY:
		if (versal_puf_regenerate(&puf_data, &cfg)) {
			key = "XSECURE_AES_PUF_KEY [regenerate]";
			goto error;
		}
		return true;
				 ...
				 uint8_t *dst, size_t dst_len)
{
	...
	if (versal_secure_key(key_id)) {
		...
		/* TODO for AMD :
		 *  check if the board is booting secure firmware,
		 *  fail otherwise
		 */
		goto secure;
	}
...
secure:
	...
	cmd.ibuf[0].buf = input;
	cmd.ibuf[0].len = input_cmd.alloc_len;
	cmd.ibuf[1].buf = p.buf;
	cmd.ibuf[1].len = p.alloc_len;
